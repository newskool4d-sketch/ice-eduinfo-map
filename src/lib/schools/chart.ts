...
export function schoolChartMetric(indicator: string, issueMetric?: string, facts?: EducationIssuesFile | null): SchoolChartMetric | null {
  if (issueMetric) {
    if (issueMetric === "special-schools") return { label: "특수학교 수", unit: "교", value: s => s.level === "special" && !s.branch ? 1 : 0 };
  ...
  switch (indicator) {
    ...
    case "schools_total": return { label: "학교수", unit: "교", value: s => s.branch ? 0 : 1 };
    case "small_schools": return { label: "소규모학교 수", unit: "교", value: s => s.branch ? 0 : s.students === null ? null : s.small ? 1 : 0 };
    case "zero_entrant_schools": return facts ? { label: "신입생 0명 학교수", unit: "교", value: s => s.branch ? 0 : facts.schools[s.id]?.entrants == null ? null : facts.schools[s.id].entrants === 0 ? 1 : 0 } : null;
...
export function chartHeight(value: number | null, maximum: number, zoom: number): number {
