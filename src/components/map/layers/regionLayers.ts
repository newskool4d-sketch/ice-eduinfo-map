...
/** 시군 바닥판(`footprint`) 채움 색이자 바닥판(plate) 색 — 비선택 시군은 이 색 쪽으로 45% 페이드(라이트 테마: 어둡게 하면 탁해진다). 단일 원천: 바닥판 `getFillColor` 도 이 상수를 쓴다. */
const PLATE: RGB = [255, 252, 246];
 ...
 * layer: a paper-colored plate (`PLATE` [255,252,246] — 밝은 디오라마, spec §4) with a
 ...
 * on it); every OTHER region fades `UNSELECTED_FADE` toward `PLATE`, which
...
function selectionAwareFillColor(code: string, fillColorOf: (code: string) => RGBA, selectedCode: string | null): RGBA {
  ...
  const [fr, fg, fb] = mix([r, g, b], PLATE, UNSELECTED_FADE);
  return [fr, fg, fb, a];
}
