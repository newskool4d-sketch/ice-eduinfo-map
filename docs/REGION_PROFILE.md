...
인천에서는 폐교·전북 시계열·정책 자료를 요청하지 않는다. 미확보 폐교 자료는 null로 표현하고 메뉴와 상세 목록을 숨긴다. 학교 위치·군구·읍면동·추세는 인천 전용 경로를 사용한다. 인천은 처음에 평면으로 열리며 지도 설정에서 입체 현황판을 선택하면 VWorld 건물을 조회한다. 건물은 서버용 키·등록 도메인 설정이 필요하고, 학교 원통 표시는 별도로 사용할 수 있다. 지도 이동 범위와 건물 조회 허용 범위는 서해 도서를 포함한다. 화면 버전·밝은/어두운 테마·글자 크기를 선택할 수 있다. 건물 설정과 검증 범위는 [인천 건물 연결](INCHEON_BUILDINGS.md)을 참고한다.
