...
test("검색·학교급·시군 필터가 목록과 지도에 함께 적용된다", async ({
  ...
  await expect(page.getByTestId("school-result-count")).toHaveText(
    "검색 결과 753개 · 지도 표시 가능 742개",
  );
  expect(await count(page)).toBe(742);
  await page
...
test("위치 없는 학교도 검색과 상세 조회가 가능하다", async ({ page }) => {
  await page.goto("/");
  await page
    .getByRole("group", { name: "학교급 필터" })
    ...
    .click();
  await expect(page.getByTestId("school-result-count")).toHaveText(
    "검색 결과 11개 · 지도 표시 가능 0개",
  );
  await page.locator('[data-testid^="school-row-"]').first().click();
  await expect(
    page.getByText("위치 자료 없음 · 지도에 표시할 수 없습니다."),
  ).toBeVisible();
