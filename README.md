...
인천 학교·분교 562개 기록, 8개 지표와 2022~2026년 인천 전체·군구·학교별 추세를 제공한다. 학교급별 추세, 학교 검색, 지도 묶음 선택, 화면 버전·밝은/어두운 테마·글자 크기 선택을 지원한다. 통계 기준일은 2026-04-01이며, 현행 지도 경계와 과거 통계의 군구 기준을 구분한다.

`지도 설정 → 배경지도`에서 자동·일반·백지도·야간·위성·끄기를 선택한다. 기본 자동 모드는 라이트 화면에서 백지도, 다크 화면에서 야간지도를 사용한다. 직접 선택한 지도는 테마 변경과 새로고침 후에도 유지된다. 위성지도에는 도로·지명 레이어를 함께 표시하고, 학교·군구 이름표와 확대 버튼도 화면 테마에 맞춘다.
...
Vercel에서는 저장소 루트를 선택하고 Node.js 22, Framework `Next.js`, Build Command `npm run build`를 사용한다. 환경변수 `NEXT_PUBLIC_EDU_MAP_PROFILE=incheon`을 Preview/Production/Development에 지정한다. 데이터는 `public/data/incheon/`의 커밋된 정적 파일을 사용하므로 배포 중 원자료 다운로드가 필요하지 않다. 환경변수를 지정하지 않은 기존 앱은 전북 프로필을 유지한다.

[프로필·재생성 안내](docs/REGION_PROFILE.md)와 [자료 검증 결과 및 남은 확인](docs/INCHEON_VERIFICATION.md)을 참고한다.

## 공통 기능과 전북 예시

시도별 학교와 시군구 교육통계를 살펴보는 오픈소스 Next.js 대시보드입니다. 이 저장소는 **전북특별자치도** profile을 첫 예시로 포함합니다. 밝은 모형 도시를 배경으로 선택한 교육지표만 색으로 강조합니다.

기본 화면은 학생 분포입니다. 넓게 보면 학교별 학생수를 가중한 상대 집중도를, 줌 13부터는 학교별 크기와 색을 보여줍니다. 학급당·교원당 학생수는 고정 크기 점의 색으로, 지역 집계는 시군 면의 색으로 표시합니다. 배경지도 종류는 지도 설정에서 선택하며, 전북의 입체 화면에서는 가까이 확대하면 건물 윤곽을 약 18% 불투명도로 표시합니다. 학교 이름은 줌 15부터 나타나고 선택한 학교는 먼저 표시합니다.

상단의 `학생 분포 · 교육여건 · 작은학교 · 특수교육 · 지역 변화`로 주제를 바꾸고 `전체 지표`에서 18개 지표를, `교육문제`에서 10개 질문을 선택합니다. `학교·통계`에서 검색·학교 상세·시군 비교를 엽니다. 목록과 학교 지도는 같은 필터를 쓰며 패널을 바꿔도 선택 지표가 유지됩니다. 1024px 미만에서는 패널을 하단에서 열고, WebGL 사용 불가·지도 오류일 때는 표로 대체합니다.
...
3. 배경지도를 표시하려면 `NEXT_PUBLIC_VWORLD_KEY`를 설정합니다. 키가 없어도 학교·경계·통계 기능은 동작합니다. 지표·경계·학교 데이터는 저장소의 `public/data/` 정적 파일을 사용합니다. 인천은 평면 지도를 사용하며, 전북은 지도 설정에서 입체·평면을 선택할 수 있습니다.
   ...
   - Vercel 프로젝트 설정 → Environment Variables 에 `NEXT_PUBLIC_VWORLD_KEY`를 Config 유형으로 추가하고 재배포합니다(Production/Preview 모두 필요하면 각각 등록). 이 변수는 브라우저 지도 요청에 사용되므로 배포 도메인을 인증키 관리에 등록해야 합니다. `vercel env add NEXT_PUBLIC_VWORLD_KEY production` 로도 등록할 수 있습니다.
   - 로컬 개발은 `.env.local`(`.gitignore`됨 — 커밋되지 않음)에 같은 키를 넣으면 됩니다.
   - 브이월드는 잘못된 키에도 HTTP 200과 XML 오류 본문을 반환할 수 있습니다. 이미지 해석 실패까지 감지해 지도에 안내와 재시도 버튼을 표시합니다. 실패해도 학교·통계는 계속 이용할 수 있습니다.
...
- **배경지도: 국토교통부 브이월드(VWorld) 오픈API** (`Base`·`white`·`midnight`·`Satellite`·`Hybrid` WMTS 타일) — 브라우저에서 직접 호출하며 지도에 출처를 표시합니다. [WMTS 공식 문서](https://www.vworld.kr/dev/v4dv_wmtsguide_s001.do)
...
- **배경 타일 지도** — 브이월드 일반·백지도·야간·위성지도와 테마 자동 연동을 제공합니다. `NEXT_PUBLIC_VWORLD_KEY`가 필요합니다.
